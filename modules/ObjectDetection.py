...
def run(dirpath, out_dir, dataset, batch_size, num_workers, device):
    ...
    for i, (imgs, labels) in tqdm(enumerate(imageloader)):
        ...
        try:
            with torch.no_grad():
                res = model(imgs)
            # convert tensor output to tensor of bounding boxes, confidence, class
            res = non_max_suppression(res)  # list of tensors
            res = np.asarray(
                [r.detach().cpu().numpy() for r in res], dtype=object
            )  # list of np arrays of different shapes
